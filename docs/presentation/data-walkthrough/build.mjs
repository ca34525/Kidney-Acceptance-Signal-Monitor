...
const { finalizePresentation, makeNativeBulletParagraphs } = await import(pathToFileURL(path.join(PRESENTATIONS_SKILL_DIR, 'container_tools/artifact_tool_utils.mjs')));
...
proto.slides = [2, 9, 2, 9, 2, 2].map((sourceIndex, i) => {
...
const finalPath = path.join(delivery, 'data-walkthrough.pptx');
...
await finalizePresentation({ workspaceDir: output, candidatePath: draft, finalPath,
  ...
  layoutArgs: ['--expected-slide-size-emu', '12192000,6858000', '--validate-bullet-geometry', '--validate-heading-fit', '--require-native-table-slide', '2', '--require-native-table-slide', '4'],
  explicitTotalSlideCount: 6, requiredNativeTableOwnerSlides: [2, 4], requiredNativeChartOwnerSlides: [],
...
console.log(JSON.stringify({ finalPath, output, slides: 6 }));
